#ifndef _CTB_HMAC_SHA2_H
#define _CTB_HMAC_SHA2_H

#ifndef CTB_SHA2_IMPLEMENTATION
	#define CTB_SHA2_IMPLEMENTATION
#endif
#ifdef CTB_HMAC_SHA2_NOPREFIX
...
#ifdef CTB_HMAC_SHA2_IMPLEMENTATION
#include <string.h>
...
#ifdef CTB_HMAC_SHA2_TEST_VECTORS
...
int main(void)
{
    ...
    unsigned char mac[_CTB_SHA512_DIGEST_SIZE];
    ...
    for (i = 0; i < 7; i++) {
        if (i != 4) {
            mac_224_size = _CTB_SHA224_DIGEST_SIZE;
            mac_256_size = _CTB_SHA256_DIGEST_SIZE;
            mac_384_size = _CTB_SHA384_DIGEST_SIZE;
            mac_512_size = _CTB_SHA512_DIGEST_SIZE;
        } else {
        ...
        ctb_hmac_sha224(keys[i], keys_len[i], (unsigned char *) messages[i],
                    strlen(messages[i]), mac, mac_224_size);
        test(vectors[i], mac, mac_224_size);
        ctb_hmac_sha256(keys[i], keys_len[i], (unsigned char *) messages[i],
                    strlen(messages[i]), mac, mac_256_size);
        test(vectors[7 + i], mac, mac_256_size);
        ctb_hmac_sha384(keys[i], keys_len[i], (unsigned char *) messages[i],
                    strlen(messages[i]), mac, mac_384_size);
        test(vectors[14 + i], mac, mac_384_size);
        ctb_hmac_sha512(keys[i], keys_len[i], (unsigned char *) messages[i],
...
#endif /* !HMAC_SHA2_IMPLEMENTATION */
